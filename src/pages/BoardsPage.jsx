...
export default function BoardsPage() {
    ...
    const nextOrderInCol = (colId) =>
        tasks.filter((t) => t.columnId === colId).reduce((m, t) => Math.max(m, t.order), -1) + 1;
    ...
    const addTask = (t) => {
        const newTask = {
            ...
            columnId: currentCol,
            ...t,
            order: nextOrderInCol(currentCol),
            sub: [],
        };
    ...
    const toggleDone = async (id) => {
        ...
        if (me?.sub?.length) {
            updated = raw.map((t) =>
                t.id === me.id ? { ...t, sub: t.sub.map((s) => ({ ...s, done: !t.done })) } : t
            );
        }
        setTasks(updated);
        ...
        <div className="flex h-screen overflow-hidden">
            <Sidebar
                ...
                gotoTask={(colId, taskId) => {
                    setColumns((cols) =>
                        cols.map((c) => (c.id === colId ? { ...c, collapsed: false } : c))
                    );
                    setCurrentCol(colId);
                    ...
                    setTimeout(() => setSelectedTaskId(null), 2000);
                }}
            />
            <main className="flex-1 flex flex-col bg-white overflow-hidden">
                ...
                {isModalOpen && (
                    <TaskModal
                        isOpen={isModalOpen}
                        onClose={closeModal}
                        onCreate={(t) => addTask({ ...t, order: nextOrderInCol(currentCol) })}
                        onUpdate={updateTask}
                        columnId={currentCol}
                        task={editingTask}
                    />
